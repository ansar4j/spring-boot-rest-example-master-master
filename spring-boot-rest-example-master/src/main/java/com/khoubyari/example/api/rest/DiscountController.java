...
import com.khoubyari.example.service.DiscountService;
...
@Api(tags = { "discount" })
public class DiscountController extends AbstractRestHandler {
	...
	public @ResponseBody Discount getDiscount(
		...
		String customerType = discountService.getCustomerType(customerID);
		// to get Customer Join date
		Date customerJoinDate = discountService.getJoinDate(customerID);
		...
		if (customerType != null) {
			discountPercent = discountService.getDiscountPercentage(customerType);
		}
		// if the user is customer more than 2 years
		else if (noOfYears > 2) {

			discountPercent = discountService.getDiscountPercentage("L");

		}
		// if the user is non customer
		if (customerType == null) {
			discountPercent = discountService.getDiscountPercentage("O");
		}
